	     ...
	     loff_t *f_pos)
{
  ...
  if(sizeof(buf) != 4){
    printk("Writing %d bytes to device.\n", sizeof(buf));
    return -EINVAL;
  }
  rup = 0;
  ret = 0;
  ...
  fail = copy_from_user(dev->data, buf, sizeof(buf));
  ...
  if(w < 0){
    printk("Negative value written. No sleeping\n");
    return 0;
  }

  time = jiffies;
  mutex_unlock(&dev->sleepy_mutex);
  fail = wait_event_interruptible_timeout(dev->wait, dev->flag != 0, w*HZ)
  if(fail){
    rup = 1;
  }
